...
## Public API (current)
- `open_device(driver, sample_rate, channels)`: attach a configured DAQ driver to the pipeline.
- `configure_acquisition(sample_rate=None, channels=None, chunk_size=None, filter_settings=None, trigger_cfg=None)`: push filter/trigger/channel updates into the pipeline.
...
- `set_listen_output_device(device_key)`: persist the audio output preference (and, as audio routing migrates here, reconfigure playback).

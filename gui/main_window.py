...
class MainWindow(QtWidgets.QMainWindow):
    ...
    dataTick = QtCore.Signal(object)

    def _open_spectrogram_tab(self, channel_id: Optional[int] = None) -> None:
        """Open a spectrogram tab for the requested channel (or active channel)."""
        try:
        ...
        # 1) Use the requested channel if available, otherwise active channel
        ...
        if channel_id is None:
            QtWidgets.QMessageBox.information(
                ...
                (
                    ...
                    "Then click the Spectrogram button again."
                ),
            )
            return
        ...
        for idx in range(tabs.count()):
            ...
            if isinstance(widget, SpectrogramTab) and getattr(widget, "_channel_id", None) == channel_id:
    ...
    def _on_dispatcher_tick(self, payload: object) -> None:
        ...
        # NEW: broadcast tick data to extra tabs (spectrogram, etc.)
        try:
            self.dataTick.emit(
                {
                    "data": data,

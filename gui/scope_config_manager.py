...
class ScopeConfigManager:
    ...
    def channel_config_from_dict(
        ...
        try:
            ...
            cfg.vertical_span_v = float(payload.get("vertical_span_v", 1.0))
            cfg.screen_offset = float(payload.get("screen_offset", 0.5))
            cfg.notch_enabled = bool(payload.get("notch_enabled", False))
            cfg.notch_freq_hz = float(payload.get("notch_freq_hz", 60.0))
            cfg.highpass_enabled = bool(payload.get("highpass_enabled", False))
            cfg.highpass_hz = float(payload.get("highpass_hz", 10.0))
            cfg.lowpass_enabled = bool(payload.get("lowpass_enabled", False))
            cfg.lowpass_hz = float(payload.get("lowpass_hz", 1_000.0))
